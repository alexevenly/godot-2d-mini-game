...
func spawn(grid: Array, offset: Vector2, cell_size: float, main_scene) -> void:
	...
	for y in range(rows + 1):
		var x := 0
		while x < cols:
			while x < cols and not _needs_horizontal_wall(grid, y, x): x += 1
			if x >= cols: break
			var start = x
			while x < cols and _needs_horizontal_wall(grid, y, x): x += 1
			var span = x - start
			if span <= 0: continue
	...
	for column in range(cols + 1):
		var y := 0
		while y < rows:
			while y < rows and not _needs_vertical_wall(grid, column, y): y += 1
			if y >= rows: break
			var start = y
			while y < rows and _needs_vertical_wall(grid, column, y): y += 1
			var span = y - start
			if span <= 0: continue
...
func _needs_horizontal_wall(grid: Array, y: int, x: int) -> bool:
	...
	if y <= 0: return not grid[0][x]
	if y >= rows: return not grid[rows - 1][x]
...
func _needs_vertical_wall(grid: Array, x: int, y: int) -> bool:
	...
	if x <= 0: return not grid[y][0]
	if x >= cols: return not grid[y][cols - 1]
